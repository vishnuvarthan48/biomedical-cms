...
function TenantStatusBadge({ status }: { status: TenantStatus }) {
  const config = {
    Active: { bg: "bg-[#10B981]/10", text: "text-[#10B981]", border: "border-[#10B981]/30" },
    Draft: { bg: "bg-[#F59E0B]/10", text: "text-[#F59E0B]", border: "border-[#F59E0B]/30" },
    Suspended: { bg: "bg-[#EF4444]/10", text: "text-[#EF4444]", border: "border-[#EF4444]/30" },
  }[status]
  return (
    <Badge variant="outline" className={cn("text-sm font-semibold border px-2.5 py-0.5", config.bg, config.text, config.border)}>
      {status}
    </Badge>
  )
}
...
// == VIEW: Tenant List ==
function TenantListView({ tenants, onView, onCreate }: {
  tenants: Tenant[], onView: (t: Tenant) => void, onCreate: () => void
}) {
  ...
  const statusCounts = { All: tenants.length, Active: tenants.filter(t => t.status === "Active").length, Draft: tenants.filter(t => t.status === "Draft").length, Suspended: tenants.filter(t => t.status === "Suspended").length }

  return (
    <div className="flex flex-col gap-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-extrabold text-foreground">Tenant Management</h2>
          <p className="text-base text-muted-foreground mt-1">Manage all tenants on the platform</p>
        </div>
        <Button className="text-white border-0 text-sm font-semibold h-10 px-5" style={{ background: "linear-gradient(135deg, #00BCD4, #00838F)" }} onClick={onCreate}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {(["All", "Active", "Draft", "Suspended"] as const).map(s => (
          <Card key={s} className={cn("cursor-pointer transition-all border-2", filterStatus === s ? "border-[#00BCD4] shadow-lg" : "border-border hover:border-[#00BCD4]/30")} onClick={() => setFilterStatus(s)}>
            <CardContent className="p-4 flex items-center gap-3">
              <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0", s === "All" ? "bg-[#00BCD4]/10" : s === "Active" ? "bg-[#10B981]/10" : s === "Draft" ? "bg-[#F59E0B]/10" : "bg-[#EF4444]/10")}>
                {s === "All" ? <Building2 className="w-5 h-5 text-[#00BCD4]" /> : s === "Active" ? <CheckCircle2 className="w-5 h-5 text-[#10B981]" /> : s === "Draft" ? <FileText className="w-5 h-5 text-[#F59E0B]" /> : <Ban className="w-5 h-5 text-[#EF4444]" />}
              </div>
              <div>
                <p className="text-2xl font-extrabold text-foreground">{statusCounts[s]}</p>
                <p className="text-xs font-semibold text-muted-foreground">{s === "All" ? "Total" : s}</p>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <Input placeholder="Search by tenant name or code..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 h-10" />
      ...
      <Card className="border border-border shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-border" style={{ background: "rgba(0,188,212,0.03)" }}>
                <th className="text-left py-4 px-5 font-bold text-foreground text-sm">Tenant Name</th>
                ...
                <th className="text-left py-4 px-5 font-bold text-foreground text-sm">Primary Contact</th>
                <th className="text-left py-4 px-5 font-bold text-foreground text-sm">Created</th>
                <th className="text-right py-4 px-5 font-bold text-foreground text-sm">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(t => (
                <tr key={t.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => onView(t)}>
                  <td className="py-4 px-5">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: "linear-gradient(135deg, rgba(0,188,212,0.1), rgba(0,188,212,0.05))" }}>
                        <Building2 className="w-4 h-4 text-[#00BCD4]" />
                      </div>
                      <span className="text-sm font-bold text-foreground">{t.name}</span>
                    </div>
                  </td>
                  <td className="py-4 px-5 text-sm font-medium text-muted-foreground font-mono">{t.code}</td>
                  <td className="py-4 px-5"><TenantStatusBadge status={t.status} /></td>
                  <td className="py-4 px-5 text-sm text-foreground">{t.primaryContact || "-"}</td>
                  <td className="py-4 px-5 text-sm text-muted-foreground">{t.createdDate}</td>
                  <td className="py-4 px-5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-[#00BCD4]" onClick={e => { e.stopPropagation(); onView(t) }}><Eye className="w-4 h-4" /></Button>
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground" onClick={e => e.stopPropagation()}><MoreVertical className="w-4 h-4" /></Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-52">
                          <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => onView(t)}><Eye className="w-4 h-4" /> View Details</DropdownMenuItem>
                          <DropdownMenuItem className="gap-2 cursor-pointer"><Pencil className="w-4 h-4" /> Edit Tenant</DropdownMenuItem>
                          <DropdownMenuSeparator />
                          {t.status === "Draft" && <DropdownMenuItem className="gap-2 cursor-pointer text-[#10B981]"><Power className="w-4 h-4" /> Activate</DropdownMenuItem>}
                          {t.status === "Active" && <DropdownMenuItem className="gap-2 cursor-pointer text-[#EF4444]"><Ban className="w-4 h-4" /> Suspend</DropdownMenuItem>}
                          {t.status === "Suspended" && <DropdownMenuItem className="gap-2 cursor-pointer text-[#10B981]"><Power className="w-4 h-4" /> Re-activate</DropdownMenuItem>}
                          <DropdownMenuItem className="gap-2 cursor-pointer"><KeyRound className="w-4 h-4" /> Reset Admin Password</DropdownMenuItem>
                          <DropdownMenuItem className="gap-2 cursor-pointer"><ScrollText className="w-4 h-4" /> Audit Logs</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </div>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="py-16 text-center">
                  ...
                  <p className="text-sm font-semibold text-muted-foreground">No tenants found</p>
                </td></tr>
              )}
            </tbody>
          </table>
...
// == VIEW: Create Tenant ==
function CreateTenantView({ onBack, onSave }: { onBack: () => void, onSave: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={onBack} className="text-muted-foreground hover:text-foreground text-sm font-semibold"><ArrowLeft className="w-4 h-4 mr-2" /> Back</Button>
        <div className="h-6 w-px bg-border" />
        <div>
          <h2 className="text-2xl font-extrabold text-foreground">Create New Tenant</h2>
          <p className="text-sm text-muted-foreground">Create a new tenant in Draft status with Tenant Group Admin credentials</p>
        </div>
      </div>
        ...
        <Card className="border-2 border-[#10B981]/30 shadow-sm" style={{ background: "rgba(16,185,129,0.03)" }}>
          <CardContent className="p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-foreground">Tenant Created Successfully</h3>
                <p className="text-sm text-muted-foreground">Save the credentials below. The password is shown only once.</p>
              </div>
            </div>
            <div className="rounded-xl border border-border bg-card p-5">
              <h4 className="text-sm font-bold text-foreground mb-4">Tenant Group Admin Login Credentials</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold text-foreground">{adminEmail || "-"}</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <code className="flex-1 px-3 py-2 rounded-lg bg-muted border border-border text-sm font-mono font-bold text-foreground">{adminUsername}</code>
                    <Button variant="outline" size="sm" className="h-9 px-2.5" onClick={() => handleCopy(adminUsername)}>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <code className="flex-1 px-3 py-2 rounded-lg bg-muted border border-border text-sm font-mono font-bold text-foreground">{generatedPassword}</code>
                    <Button variant="outline" size="sm" className="h-9 px-2.5" onClick={() => handleCopy(generatedPassword || "")}>
              ...
              <div className="flex items-center gap-2 mt-4 px-3 py-2 rounded-lg bg-[#F59E0B]/5 border border-[#F59E0B]/20">
                ...
                <p className="text-xs text-[#F59E0B] font-medium">MustChangePassword = true. The admin will be forced to change this password on first login.</p>
              </div>
            </div>
            <div className="flex items-center gap-3 mt-5">
              <Button className="text-white border-0 text-sm font-semibold h-10 px-5" style={{ background: "linear-gradient(135deg, #00BCD4, #00838F)" }} onClick={onSave}>Done</Button>
              <Button variant="outline" className="text-sm font-semibold h-10 px-4" onClick={() => { setSubmitted(false); setGeneratedPassword(null) }}>Create Another</Button>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {!submitted && (
        <>
          ...
          <Card className="border border-border shadow-sm">
            <CardContent className="p-6">
              <h3 className="text-base font-extrabold text-foreground mb-5 pb-3 border-b border-border">Tenant Details</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="flex flex-col gap-2">
                  <Label className="text-sm font-bold text-foreground">Tenant Name <span className="text-[#EF4444]">*</span></Label>
                  <Input className="h-10" placeholder="e.g. Apollo Hospitals Group" />
                </div>
                <div className="flex flex-col gap-2">
                  <Label className="text-sm font-bold text-foreground">Tenant Code / Slug <span className="text-[#EF4444]">*</span></Label>
                  <Input className="h-10 font-mono" placeholder="e.g. apollo-hospitals (URL-safe)" />
                </div>
                <div className="flex flex-col gap-2">
                  <Label className="text-sm font-bold text-foreground">Timezone</Label>
                  <Select><SelectTrigger className="h-10"><SelectValue placeholder="Asia/Kolkata (default)" /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="flex flex-col gap-2">
                  <Label className="text-sm font-bold text-foreground">Locale</Label>
                  <Select><SelectTrigger className="h-10"><SelectValue placeholder="Select locale" /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Input type="email" className="h-10" placeholder="admin@hospital.com" />
                </div>
                <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-2 md:col-span-2">
                  ...
                  <Textarea className="min-h-[80px]" placeholder="Any additional information..." />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border border-border shadow-sm">
            <CardContent className="p-6">
              <div className="flex items-center gap-3 mb-5 pb-3 border-b border-border">
                ...
                <div>
                  <h3 className="text-base font-extrabold text-foreground">Tenant Group Admin Account</h3>
                  <p className="text-xs text-muted-foreground">This user will be the first admin for the tenant. They can manage organizations, users, roles, and privileges.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="flex flex-col gap-2">
                  ...
                  <Input className="h-10" placeholder="e.g. Dr. Rajesh Kumar" value={adminName} onChange={e => setAdminName(e.target.value)} />
                </div>
                <div className="flex flex-col gap-2">
                  <Label className="text-sm font-bold text-foreground">Admin Email <span className="text-[#EF4444]">*</span></Label>
                  <Input type="email" className="h-10" placeholder="e.g. rajesh.kumar@hospital.com" value={adminEmail} onChange={e => setAdminEmail(e.target.value)} />
                </div>
                <div className="flex flex-col gap-2">
                  <Label className="text-sm font-bold text-foreground">Username <span className="text-[#EF4444]">*</span></Label>
                  <Input className="h-10 font-mono" placeholder="e.g. rajesh.kumar" value={adminUsername} onChange={e => setAdminUsername(e.target.value)} />
              ...
              <div className="mt-5 pt-4 border-t border-border">
                ...
                <div className="flex items-center gap-4 mb-4">
                  <label className="flex items-center gap-2 px-4 py-2.5 rounded-lg border cursor-pointer transition-colors text-sm font-semibold"
                    style={{ borderColor: useAutoPassword ? "rgba(0,188,212,0.4)" : "var(--border)", background: useAutoPassword ? "rgba(0,188,212,0.05)" : "transparent", color: useAutoPassword ? "#00BCD4" : "var(--muted-foreground)" }}
                    onClick={() => setUseAutoPassword(true)}>
                    <input type="radio" name="pwMode" checked={useAutoPassword} onChange={() => setUseAutoPassword(true)} className="accent-[#00BCD4]" />
                    Auto-generate password
                  </label>
                  <label className="flex items-center gap-2 px-4 py-2.5 rounded-lg border cursor-pointer transition-colors text-sm font-semibold"
                    style={{ borderColor: !useAutoPassword ? "rgba(0,188,212,0.4)" : "var(--border)", background: !useAutoPassword ? "rgba(0,188,212,0.05)" : "transparent", color: !useAutoPassword ? "#00BCD4" : "var(--muted-foreground)" }}
                    onClick={() => setUseAutoPassword(false)}>
                    <input type="radio" name="pwMode" checked={!useAutoPassword} onChange={() => setUseAutoPassword(false)} className="accent-[#00BCD4]" />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="relative">
                        <Input type={showPassword ? "text" : "password"} className="h-10 pr-10 font-mono" placeholder="Min 8 chars, uppercase, number, special"
                      ...
                      {adminPassword.length > 0 && (
                        <div className="flex flex-col gap-1 mt-1">
                          {[
                            { label: "At least 8 characters", valid: adminPassword.length >= 8 },
                            { label: "Uppercase letter", valid: /[A-Z]/.test(adminPassword) },
                            { label: "Number", valid: /[0-9]/.test(adminPassword) },
                            { label: "Special character", valid: /[^A-Za-z0-9]/.test(adminPassword) },
                          ].map(rule => (
          ...
          <div className="flex items-center gap-3">
            <Button className="text-white border-0 text-sm font-semibold h-10 px-5" style={{ background: "linear-gradient(135deg, #00BCD4, #00838F)" }}
              onClick={handleCreate} disabled={!adminUsername.trim() || !adminEmail.trim() || (!useAutoPassword && adminPassword.length < 8)}>
...
// == VIEW: Tenant Details ==
function TenantDetailView({ tenant, onBack }: { tenant: Tenant, onBack: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={onBack} className="text-muted-foreground hover:text-foreground text-sm font-semibold"><ArrowLeft className="w-4 h-4 mr-2" /> Back to Tenants</Button>
        <div className="h-6 w-px bg-border" />
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h2 className="text-2xl font-extrabold text-foreground">{tenant.name}</h2>
            <TenantStatusBadge status={tenant.status} />
          </div>
          <p className="text-sm text-muted-foreground font-mono">{tenant.code}</p>
        </div>
        <div className="flex items-center gap-2">
          {tenant.status === "Draft" && <Button className="text-white border-0 text-sm font-semibold h-10 px-4 bg-[#10B981] hover:bg-[#059669]"><Power className="w-4 h-4 mr-1.5" /> Activate</Button>}
          {tenant.status === "Active" && <Button variant="outline" className="text-sm font-semibold h-10 px-4 text-[#EF4444] border-[#EF4444]/30 hover:bg-[#EF4444]/5"><Ban className="w-4 h-4 mr-1.5" /> Suspend</Button>}
          {tenant.status === "Suspended" && <Button className="text-white border-0 text-sm font-semibold h-10 px-4 bg-[#10B981] hover:bg-[#059669]"><Power className="w-4 h-4 mr-1.5" /> Re-activate</Button>}
          <Button variant="outline" className="text-sm font-semibold h-10 px-4"><Pencil className="w-4 h-4 mr-1.5" /> Edit</Button>
        </div>
      </div>
      ...
      <Card className="border border-border shadow-sm">
        <CardContent className="p-6">
          <h3 className="text-base font-extrabold text-foreground mb-4 pb-3 border-b border-border">Tenant Profile</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <div className="flex items-start gap-3"><Globe className="w-5 h-5 text-[#00BCD4] mt-0.5 flex-shrink-0" /><div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Timezone</p><p className="text-sm font-bold text-foreground">{tenant.timezone}</p></div></div>
            <div className="flex items-start gap-3"><Mail className="w-5 h-5 text-[#00BCD4] mt-0.5 flex-shrink-0" /><div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Contact Email</p><p className="text-sm font-bold text-foreground">{tenant.contactEmail || "-"}</p></div></div>
            <div className="flex items-start gap-3"><Phone className="w-5 h-5 text-[#00BCD4] mt-0.5 flex-shrink-0" /><div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Contact Phone</p><p className="text-sm font-bold text-foreground">{tenant.contactPhone || "-"}</p></div></div>
            <div className="flex items-start gap-3"><Calendar className="w-5 h-5 text-[#00BCD4] mt-0.5 flex-shrink-0" /><div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Created</p><p className="text-sm font-bold text-foreground">{tenant.createdDate}</p></div></div>
            <div className="flex items-start gap-3"><Users className="w-5 h-5 text-[#00BCD4] mt-0.5 flex-shrink-0" /><div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Primary Contact</p><p className="text-sm font-bold text-foreground">{tenant.primaryContact || "-"}</p></div></div>
            {tenant.notes && <div className="flex items-start gap-3 md:col-span-3"><FileText className="w-5 h-5 text-[#00BCD4] mt-0.5 flex-shrink-0" /><div><p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Notes</p><p className="text-sm text-foreground">{tenant.notes}</p></div></div>}
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border border-border shadow-sm">
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-4 pb-3 border-b border-border">
            <h3 className="text-base font-extrabold text-foreground">Tenant Group Admins</h3>
            <Button className="text-white border-0 text-sm font-semibold h-9 px-4" style={{ background: "linear-gradient(135deg, #00BCD4, #00838F)" }} onClick={() => setShowCreateAdmin(true)}>
          ...
          {showCreateAdmin && (
            <div className="rounded-xl border-2 border-dashed border-[#00BCD4]/40 bg-[#00BCD4]/5 p-5 mb-4">
              <h4 className="text-sm font-bold text-foreground mb-4">Create Tenant Group Admin</h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div className="flex flex-col gap-2"><Label className="text-sm font-bold text-foreground">Email <span className="text-[#EF4444]">*</span></Label><Input type="email" className="h-10" placeholder="admin@hospital.com" /></div>
              ...
              <div className="flex flex-col gap-2 mb-4">
                ...
                <Input className="h-10 bg-muted/30" value="Tenant Group Admin" readOnly />
              </div>
              ...
              {generatedPassword && (
                <div className="rounded-lg border border-[#10B981]/30 bg-[#10B981]/5 p-4 mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    <CheckCircle2 className="w-5 h-5 text-[#10B981]" />
                    <p className="text-sm font-bold text-[#10B981]">Admin Created Successfully</p>
                  </div>
                  <p className="text-xs text-muted-foreground mb-2">Default password (shown once only - copy it now):</p>
                  <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs text-muted-foreground mt-2">User must change password on first login (MustChangePassword = true)</p>
                </div>
              )}

              <div className="flex items-center gap-2">
                {!generatedPassword && (
                  <Button className="text-white border-0 text-sm font-semibold h-10 px-5" style={{ background: "linear-gradient(135deg, #00BCD4, #00838F)" }} onClick={handleCreateAdmin}>
                    <Send className="w-4 h-4 mr-1.5" /> Create Admin User
                  </Button>
                )}
                <Button variant="outline" className="text-sm font-semibold h-10 px-4" onClick={() => { setShowCreateAdmin(false); setGeneratedPassword(null) }}>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-3 px-4 font-bold text-foreground text-sm">Name</th>
                <th className="text-left py-3 px-4 font-bold text-foreground text-sm">Email</th>
                <th className="text-left py-3 px-4 font-bold text-foreground text-sm">Role</th>
                <th className="text-left py-3 px-4 font-bold text-foreground text-sm">Status</th>
                <th className="text-right py-3 px-4 font-bold text-foreground text-sm">Actions</th>
              </tr>
            </thead>
            <tbody>
              <tr className="border-b border-border last:border-0 hover:bg-muted/30">
                <td className="py-3 px-4 text-sm font-semibold text-foreground">Arjun Kumar</td>
                <td className="py-3 px-4 text-sm text-muted-foreground">arjun.kumar@apollohospitals.com</td>
                <td className="py-3 px-4"><Badge variant="outline" className="text-xs font-semibold border-[#00BCD4]/30 text-[#00BCD4]">Tenant Group Admin</Badge></td>
                <td className="py-3 px-4"><Badge variant="outline" className="text-xs font-semibold border-[#10B981]/30 text-[#10B981] bg-[#10B981]/10">Active</Badge></td>
                <td className="py-3 px-4 text-right">
                  <Button variant="ghost" size="sm" className="h-8 text-sm text-muted-foreground hover:text-[#00BCD4] gap-1.5"><KeyRound className="w-3.5 h-3.5" /> Reset Password</Button>
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <Card className="border border-border shadow-sm">
        <CardContent className="p-6">
          ...
          <div className="flex flex-col gap-0">
            {tenantLogs.length > 0 ? tenantLogs.slice(0, 5).map(log => (
...
function AuditLogsView({ logs }: { logs: AuditLog[] }) {
  ...
  const filtered = logs.filter(l => {
    const matchSearch = !search || l.summary.toLowerCase().includes(search.toLowerCase()) || l.actor.toLowerCase().includes(search.toLowerCase()) || l.entity.toLowerCase().includes(search.toLowerCase())
  ...
  return (
    <div className="flex flex-col gap-8">
      <div>
        <h2 className="text-3xl font-extrabold text-foreground">Platform Audit Logs</h2>
        <p className="text-base text-muted-foreground mt-1">Track all platform-wide actions and changes</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 max-w-md">
          <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input placeholder="Search logs..." value={search} onChange={e => setSearch(e.target.value)} className="pl-10 h-10" />
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border" style={{ background: "rgba(0,188,212,0.03)" }}>
                ...
                <th className="text-left py-4 px-5 font-bold text-foreground text-sm">Tenant</th>
            ...
            <tbody>
              {filtered.map(log => (
                <tr key={log.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="py-4 px-5 text-sm text-foreground">{log.tenant}</td>
                  ...
                  <td className="py-4 px-5">
                    <Badge variant="outline" className={cn("text-xs font-semibold",
                      ...
                      log.action === "DELETE" ? "border-[#EF4444]/30 text-[#EF4444] bg-[#EF4444]/10" :
                      log.action === "RESET_PASSWORD" ? "border-[#F59E0B]/30 text-[#F59E0B] bg-[#F59E0B]/10" :
                      "border-border"
                    )}>{log.action}</Badge>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={6} className="py-16 text-center">
                  ...
                  <p className="text-sm font-semibold text-muted-foreground">No logs found</p>
                </td></tr>
              )}
            </tbody>
          </table>
              ...
              <div className="mt-5 pt-4 border-t border-border">
                ...
                <div className="flex items-center gap-4 mb-4">
                  <label className="flex items-center gap-2 px-4 py-2.5 rounded-lg border cursor-pointer transition-colors text-sm font-semibold"
                    ...
                    <input type="radio" name="orgPwMode" checked={orgUseAutoPassword} onChange={() => setOrgUseAutoPassword(true)} className="accent-[#00BCD4]" />
                    Auto-generate password
                  </label>
                  <label className="flex items-center gap-2 px-4 py-2.5 rounded-lg border cursor-pointer transition-colors text-sm font-semibold"
                    style={{ borderColor: !orgUseAutoPassword ? "rgba(0,188,212,0.4)" : "var(--border)", background: !orgUseAutoPassword ? "rgba(0,188,212,0.05)" : "transparent", color: !orgUseAutoPassword ? "#00BCD4" : "var(--muted-foreground)" }}
                    onClick={() => setOrgUseAutoPassword(false)}>
                    <input type="radio" name="orgPwMode" checked={!orgUseAutoPassword} onChange={() => setOrgUseAutoPassword(false)} className="accent-[#00BCD4]" />
                    Set password manually
                  </label>
                </div>
    ...
    return (
      <div className="flex flex-col gap-6">
        ...
        <Card className="border border-border shadow-sm"><CardContent className="p-6">
          ...
          <p className="text-sm text-muted-foreground">Select existing tenant users and assign them as Organization Admin for this org.</p>
        </CardContent></Card>
      </div>
    )
  }
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-1 p-1 rounded-xl bg-card border border-border w-fit">
        {[{ id: "resources" as const, label: "Resources", count: seededResources.length }, { id: "actions" as const, label: "Actions", count: seededActions.length }].map(tab => (
          <button key={tab.id} onClick={() => setCatalogTab(tab.id)}
            className={cn("flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all",
              catalogTab === tab.id ? "text-white shadow-md" : "text-muted-foreground hover:text-foreground"
            )} style={catalogTab === tab.id ? { background: "linear-gradient(135deg, #00BCD4, #00838F)" } : {}}>
            {tab.label} <Badge variant="outline" className={cn("text-[10px] ml-1", catalogTab === tab.id ? "border-white/30 text-white" : "")}>{tab.count}</Badge>
          </button>
        ))}
      </div>
      <Card className="border border-border shadow-sm"><div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-border" style={{ background: "rgba(0,188,212,0.03)" }}>
...
export function PlatformAdminPage() {
  ...
  const tabs: { id: PlatformTab; label: string; icon: React.ElementType; section: string }[] = [
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "tenants":
        if (view === "create") return <CreateTenantView onBack={() => setView("list")} onSave={() => setView("list")} />
        if (view === "detail" && selectedTenant) return <TenantDetailView tenant={selectedTenant} onBack={() => { setView("list"); setSelectedTenant(null) }} />
        return <TenantListView tenants={mockTenants} onView={t => { setSelectedTenant(t); setView("detail") }} onCreate={() => setView("create")} />
      ...
      <div className="flex flex-col gap-3">
        {tabSections.map(section => (
          <div key={section} className="flex items-center gap-2">
            <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground w-24 flex-shrink-0">{section}</span>
            <div className="flex items-center gap-1 p-1 rounded-xl bg-card border border-border flex-wrap">
              {tabs.filter(t => t.section === section).map(tab => (
                <button key={tab.id} onClick={() => { setActiveTab(tab.id); if (tab.id === "tenants") setView("list") }}
                  ...
                  )} style={activeTab === tab.id ? { background: section === "Platform" ? "linear-gradient(135deg, #8B5CF6, #6D28D9)" : "linear-gradient(135deg, #00BCD4, #00838F)" } : {}}>
