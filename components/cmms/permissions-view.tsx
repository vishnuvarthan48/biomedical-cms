...
const tabs: { key: TabKey; label: string; icon: React.ReactNode }[] = [
  ...
  { key: "resources", label: "Resources", icon: <Layers className="h-4 w-4" /> },
  { key: "matrix", label: "Permission Matrix", icon: <Grid3X3 className="h-4 w-4" /> },
  { key: "user-roles", label: "User Roles", icon: <Users className="h-4 w-4" /> },
...
function RolesTab() {
  ...
  const saveEdit = () => {
    ...
    setRoles((prev) =>
      prev.map((r) =>
        r.id === editId ? { ...r, name: editName.trim(), description: editDesc.trim() } : r
      )
    )
    setEditId(null)
  }
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Define roles for your organization. Roles group permissions that can be assigned to users.
        </p>
        <button
          type="button"
      ...
      {showAdd && (
        <div className="mb-4 rounded-lg border border-primary/20 bg-primary/5 p-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-foreground">Role Code</label>
              <input
                type="text"
                value={newCode}
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card">
        <table className="w-full">
          ...
          <tbody>
            {roles.map((role) => (
              <tr key={role.id} className="border-b border-border last:border-0 hover:bg-secondary/20">
                {editId === role.id ? (
                  <>
                    ...
                    <td className="px-4 py-2 text-xs font-mono text-muted-foreground">{role.code}</td>
                    <td className="px-4 py-2">
                      <input
                        type="text"
                        value={editDesc}
                  ...
                  <>
                    ...
                    <td className="px-4 py-3 text-sm text-foreground">{role.assignedUsersCount}</td>
...
function ResourcesTab() {
  const [resources, setResources] = useState<ResourceDef[]>(initialResourcesData)
  const [showAdd, setShowAdd] = useState(false)
  ...
  const getChildren = (parentId: string) => resources.filter((r) => r.parentId === parentId)
  ...
  const toggleActive = (id: string) => {
    setResources((prev) => prev.map((r) => (r.id === id ? { ...r, isActive: !r.isActive } : r)))
  }

  const deleteResource = (id: string) => {
    setResources((prev) => prev.filter((r) => r.id !== id && r.parentId !== id))
  }
    ...
    <div>
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <div className="rounded-lg border border-border bg-card p-3">
          <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Total Resources</p>
          <p className="mt-1 text-xl font-bold text-foreground">{totalResources}</p>
        </div>
        <div className="rounded-lg border border-border bg-card p-3">
          <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Active</p>
          <p className="mt-1 text-xl font-bold text-success">{activeResources}</p>
        </div>
        <div className="rounded-lg border border-border bg-card p-3">
          <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Parent Modules</p>
          <p className="mt-1 text-xl font-bold text-foreground">{parentCount}</p>
        </div>
        <div className="rounded-lg border border-border bg-card p-3">
          <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">Sub-Resources</p>
          <p className="mt-1 text-xl font-bold text-muted-foreground">{childCount}</p>
        </div>
      </div>
      ...
      {showAdd && (
        <div className="mb-4 rounded-lg border border-primary/20 bg-primary/5 p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button type="button" onClick={resetAddForm} className="text-muted-foreground hover:text-foreground">
              <X className="h-4 w-4" />
            </button>
          </div>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <input
                ...
                className="h-9 w-full rounded-md border border-input bg-card px-3 font-mono text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-ring focus:ring-1 focus:ring-ring"
              />
              <p className="mt-1 text-[10px] text-muted-foreground">Unique identifier used in code</p>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-foreground">
              ...
              <input
                ...
                className="h-9 w-full rounded-md border border-input bg-card px-3 text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-ring focus:ring-1 focus:ring-ring"
              />
              <p className="mt-1 text-[10px] text-muted-foreground">Human-readable name shown in UI</p>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-foreground">Parent Resource</label>
              <select
                value={newParent}
              ...
              <p className="mt-1 text-[10px] text-muted-foreground">Group under a parent module</p>
            </div>
            <div>
              <label className="mb-1 block text-xs font-medium text-foreground">Description</label>
              <input
                ...
                className="h-9 w-full rounded-md border border-input bg-card px-3 text-sm text-foreground placeholder:text-muted-foreground outline-none focus:border-ring focus:ring-1 focus:ring-ring"
              />
              <p className="mt-1 text-[10px] text-muted-foreground">Describe this screen or module</p>
            </div>
          </div>
          {newParent && (
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card">
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        {isEditing ? (
                          ...
                          <div className="flex items-center gap-2">
                            <Layers className="h-3.5 w-3.5 text-primary" />
                            <span className="text-sm font-medium text-foreground">{res.resourceName}</span>
                            {children.length > 0 && (
                      ...
                      <td className="px-4 py-3">
                        ...
                        >
                          {res.isActive ? <Unlock className="h-2.5 w-2.5" /> : <Lock className="h-2.5 w-2.5" />}
                          ...
                          >
                            ...
                            <td className="py-2.5 pl-10 pr-4">
                              {isChildEditing ? (
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-sm text-foreground">{child.resourceName}</span>
                                </div>
                              )}
                            </td>
                            ...
                            <td className="px-4 py-2.5">
                              {isChildEditing ? (
                                ...
                                <span className="text-xs text-foreground">{res.resourceName}</span>
                              )}
                            </td>
                            <td className="px-4 py-2.5">
                              {isChildEditing ? (
        ...
        <div className="flex items-center justify-between border-t border-border bg-secondary/10 px-4 py-2.5">
          <p className="text-xs text-muted-foreground">
            Showing {filteredTopLevel.length} of {topLevel.length} top-level resources
            {childCount > 0 && <> with {childCount} sub-resources</>}
          </p>
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
              <span className="inline-block h-2 w-2 rounded-full bg-primary/40" /> Module
            </span>
            <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
              <span className="inline-block h-2 w-2 rounded-full bg-accent/40" /> Screen
            </span>
          </div>
        </div>
      </div>
...
function PermissionMatrixTab() {
  ...
  const [permissions, setPermissions] = useState<RolePermission[]>(initialPermissionsData)
  ...
  const isGranted = (resourceId: string, actionId: string) => {
    return permissions.some(
      (p) => p.roleId === selectedRole && p.resourceId === resourceId && p.actionId === actionId && p.isAllowed
    )
  }

  const togglePermission = (resourceId: string, actionId: string) => {
    const existing = permissions.find(
      (p) => p.roleId === selectedRole && p.resourceId === resourceId && p.actionId === actionId
    )
    if (existing) {
      setPermissions((prev) =>
        prev.map((p) =>
          p.roleId === selectedRole && p.resourceId === resourceId && p.actionId === actionId
    ...
    setPermissions((prev) => {
      ...
      for (const action of matrixActions) {
        const idx = updated.findIndex(
          (p) => p.roleId === selectedRole && p.resourceId === resourceId && p.actionId === action.id
        )
        if (idx >= 0) {
          updated[idx] = { ...updated[idx], isAllowed: true }
        } else {
          updated.push({ roleId: selectedRole, resourceId, actionId: action.id, isAllowed: true })
        }
      }
      return updated
    })
  ...
  const revokeAllForResource = (resourceId: string) => {
    setPermissions((prev) =>
      prev.map((p) =>
        p.roleId === selectedRole && p.resourceId === resourceId ? { ...p, isAllowed: false } : p
      )
    )
  }
              ...
              return (
                <Fragment key={res.id}>
                  ...
                  <tr className="border-b border-border bg-secondary/5">
                    <td className="sticky left-0 z-10 bg-secondary/5 px-4 py-2.5" colSpan={matrixActions.length + 2}>
                      <div className="flex items-center gap-2">
                        <Layers className="h-3.5 w-3.5 text-primary" />
                  ...
                  {children.map((child) => (
                    <tr key={child.id} className="border-b border-border hover:bg-secondary/20">
    ...
    <div>
      ...
      <div className="overflow-hidden rounded-lg border border-border bg-card">
        <table className="w-full">
          ...
          <tbody>
            {filtered.map((user) => {
              const userRoleIds = [...new Set(user.orgMemberships.flatMap((m) => m.roles))]
              return (
                    ...
                    {editUserId === user.id ? (
                      <div className="flex flex-wrap gap-1.5">
                        {roles.map((role) => (
                          ...
                          >
                            {editRoles.includes(role.id) && <Check className="h-2.5 w-2.5" />}
                            {role.name}
                          </button>
                        ))}
                      </div>
  ...
  return (
    <div>
      <p className="mb-4 text-sm text-muted-foreground">
        Track all permission and role assignment changes for compliance and auditing.
      </p>
      <div className="overflow-hidden rounded-lg border border-border bg-card">
        <table className="w-full">
          <thead>
          ...
          <tbody>
            {tenantLogs.map((entry) => (
              <tr key={entry.id} className="border-b border-border last:border-0 hover:bg-secondary/20">
                ...
                <td className="px-4 py-3 text-sm font-medium text-foreground">{entry.actor}</td>
                <td className="px-4 py-3">
                  <span
                    className={`inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                      changeTypeStyles[entry.action] ?? "bg-secondary text-muted-foreground"
                    }`}
                  >
                    {entry.action.replace("_", " ")}
                  </span>
                </td>
                <td className="px-4 py-3 text-sm text-foreground">{entry.entity}</td>
                <td className="px-4 py-3 text-sm text-muted-foreground">{entry.summary}</td>
              </tr>
            ))}
          </tbody>
        </table>
