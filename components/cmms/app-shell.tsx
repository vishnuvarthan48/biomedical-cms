...
import { OrgSwitcher } from "@/components/cmms/org-switcher"
...
const navItems: NavItem[] = [
  { id: MODULES.DASHBOARD, label: "Dashboard", icon: LayoutDashboard, section: "Overview", path: ROUTES.DASHBOARD },
  { id: MODULES.ASSETS, label: "Asset Registration", icon: Wrench, section: "Asset Management", path: ROUTES.ASSETS },
  { id: MODULES.DEVICES, label: "Device Management", icon: Cpu, section: "Asset Management", path: ROUTES.DEVICES },
  { id: MODULES.ASSET_TRANSFER, label: "Asset Transfer", icon: ArrowRightLeft, section: "Asset Management", path: ROUTES.ASSET_TRANSFER },
  { id: MODULES.WORK_ORDERS, label: "Work Orders", icon: ClipboardList, section: "Maintenance", path: ROUTES.WORK_ORDERS },
  { id: MODULES.PM, label: "Preventive Maintenance", icon: CalendarClock, section: "Maintenance", path: ROUTES.PM },
  { id: MODULES.CALIBRATION, label: "Calibration", icon: Gauge, section: "Maintenance", path: ROUTES.CALIBRATION },
  { id: MODULES.STORE_MASTER, label: "Store Master", icon: Warehouse, section: "Operations", path: ROUTES.STORE_MASTER },
  { id: MODULES.ITEM_MASTER, label: "Item Master", icon: Package, section: "Operations", path: ROUTES.ITEM_MASTER },
  { id: MODULES.GRN, label: "GRN", icon: ClipboardCheck, section: "Operations", path: ROUTES.GRN },
  { id: MODULES.VENDORS, label: "Vendor Registration", icon: Building, section: "Operations", path: ROUTES.VENDORS },
  { id: MODULES.REPORTS, label: "Reports", icon: BarChart3, section: "Analytics", path: ROUTES.REPORTS },
  { id: MODULES.COMPLIANCE, label: "Compliance", icon: ShieldCheck, section: "Analytics", path: ROUTES.COMPLIANCE },
  {
...
export function AppShell() {
  ...
  const activeItem = visibleNavItems.find((n) => isActiveRoute(location.pathname, n.path))
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-5 py-6">
        ...
        {!sidebarCollapsed && (
          <div className="overflow-hidden">
            ...
            <p className="text-[11px] font-bold tracking-[0.2em] uppercase" style={{ color: "#00BCD4" }}>
              CMMS Platform
            </p>
          </div>
        )}
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isActive ? "text-white" : "text-[#8899B0] hover:text-white hover:bg-white/5",
                      )}
                      style={
                        isActive
                          ? {
      ...
      <div className="p-4" style={{ borderTop: "1px solid rgba(0,188,212,0.1)" }}>
        <div className={cn("flex items-center", sidebarCollapsed ? "justify-center" : "gap-3")}>
          ...
          {!sidebarCollapsed && (
            <button onClick={logout} className="text-[#64748B] hover:text-[#EF4444] transition-colors" title="Sign out">
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: "#EDF2F7" }}>
      <aside
        className={cn("hidden lg:flex flex-col flex-shrink-0 transition-all duration-300 relative z-10", sidebarCollapsed ? "w-[72px]" : "w-[270px]")}
        style={{ background: "linear-gradient(180deg, #0D1B2A 0%, #152238 100%)", boxShadow: "4px 0 24px rgba(0,0,0,0.15)" }}
      >
        <SidebarContent />
        <button
