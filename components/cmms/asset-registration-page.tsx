...
import { Plus, Search, Filter, Download, Upload, X, ChevronDown, Pencil, Eye, Trash2, MoreVertical, Copy } from "lucide-react"
...
const categories = ["All", "Imaging", "Life Support", "Patient Monitoring", "Sterilization"]
const statuses = ["All", "Working Fine", "Under Repair", "Condemned", "Standby", "New", "Not Installed"]
const riskLevels = ["All", "Critical", "High", "Medium", "Low"]
const departments = ["All", ...Array.from(new Set(equipmentList.map(e => e.dept)))]

export function AssetRegistrationPage({ onRegister, onEdit }: { onRegister: () => void; onEdit: (assetId: string) => void }) {
  ...
  const activeFilterCount = [statusFilter, riskFilter, deptFilter].filter(f => f !== "All").length
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-foreground tracking-tight">Asset Registration</h1>
          <p className="text-base text-muted-foreground mt-1">Register, view, and manage biomedical equipment assets</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" className="h-11 text-sm font-semibold"><Upload className="w-4 h-4 mr-2" /> Import</Button>
          <Button variant="outline" className="h-11 text-sm font-semibold"><Download className="w-4 h-4 mr-2" /> Export</Button>
          <Button className="text-white border-0 h-11 text-sm font-semibold px-5" style={{ background: "linear-gradient(135deg, #00BCD4, #00838F)" }}
            onClick={onRegister}>
          ...
          <Card className="border border-border shadow-sm">
            <CardContent className="p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                {/* Risk Filter */}
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-bold text-foreground">Risk Level</label>
                  <div className="flex flex-wrap gap-1.5">
                    {riskLevels.map(r => (
                      <button key={r}
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {activeCategory !== "All" && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#00BCD4]/10 text-[#00BCD4] text-sm font-semibold border border-[#00BCD4]/20">
                ...
                <button onClick={() => setActiveCategory("All")} className="hover:text-[#00838F]"><X className="w-3.5 h-3.5" /></button>
              </span>
            )}
            {statusFilter !== "All" && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#00BCD4]/10 text-[#00BCD4] text-sm font-semibold border border-[#00BCD4]/20">
                Status: {statusFilter}
                <button onClick={() => setStatusFilter("All")} className="hover:text-[#00838F]"><X className="w-3.5 h-3.5" /></button>
              </span>
            )}
            {riskFilter !== "All" && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#00BCD4]/10 text-[#00BCD4] text-sm font-semibold border border-[#00BCD4]/20">
                Risk: {riskFilter}
                <button onClick={() => setRiskFilter("All")} className="hover:text-[#00838F]"><X className="w-3.5 h-3.5" /></button>
              </span>
            )}
            {deptFilter !== "All" && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#00BCD4]/10 text-[#00BCD4] text-sm font-semibold border border-[#00BCD4]/20">
                Dept: {deptFilter}
                <button onClick={() => setDeptFilter("All")} className="hover:text-[#00838F]"><X className="w-3.5 h-3.5" /></button>
              </span>
            )}
            {searchQuery && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#00BCD4]/10 text-[#00BCD4] text-sm font-semibold border border-[#00BCD4]/20">
                Search: &quot;{searchQuery}&quot;
                <button onClick={() => setSearchQuery("")} className="hover:text-[#00838F]"><X className="w-3.5 h-3.5" /></button>
              </span>
            )}
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-border bg-muted/30">
                  <th className="text-left py-4 px-5 font-bold text-foreground text-sm">Asset ID</th>
                  ...
                  <th className="text-left py-4 px-5 font-bold text-foreground text-sm">Risk</th>
                  <th className="text-left py-4 px-5 font-bold text-foreground text-sm">Status</th>
                  <th className="text-left py-4 px-5 font-bold text-foreground text-sm">Next PM</th>
                ...
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={9} className="py-16 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <Button variant="outline" size="sm" className="mt-2 text-sm font-semibold" onClick={clearAllFilters}>
                          Clear all filters
                        </Button>
                      </div>
                    </td>
                  ...
                  filtered.map(eq => (
                    <tr key={eq.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors cursor-pointer">
                      <td className="py-4 px-5 font-mono font-bold text-[#00BCD4] text-sm">{eq.id}</td>
                      <td className="py-4 px-5 text-foreground font-semibold text-sm max-w-[260px] truncate">{eq.name}</td>
                      <td className="py-4 px-5 text-foreground text-sm">{eq.category}</td>
                      <td className="py-4 px-5 text-foreground text-sm">{eq.dept}</td>
                      <td className="py-4 px-5 font-mono text-sm text-foreground">{eq.serial}</td>
                      <td className="py-4 px-5">
                        <span className={`inline-flex px-3 py-1 rounded-full text-sm font-semibold border ${riskColors[eq.risk]}`}>{eq.risk}</span>
                      </td>
                      <td className="py-4 px-5">
                        <span className={`inline-flex px-3 py-1 rounded-full text-sm font-semibold ${statusColors[eq.status]}`}>{eq.status}</span>
                      </td>
                      <td className={`py-4 px-5 text-sm font-semibold ${eq.nextPM === "Overdue" ? "text-[#DC2626]" : "text-foreground"}`}>
                        {eq.nextPM}
                      </td>
                      <td className="py-4 px-5 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button
                            ...
                            title="Edit Asset"
                            onClick={(e) => { e.stopPropagation(); onEdit(eq.id) }}
                          >
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-48">
                              <DropdownMenuItem className="gap-2 cursor-pointer" onClick={() => onEdit(eq.id)}>
