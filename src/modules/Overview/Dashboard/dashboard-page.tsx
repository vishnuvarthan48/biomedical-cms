...
import {
  useCreateAirTicketMutation,
  useGetListAirTicketQuery,
} from "@/src/redux/rtkQueries/airTicketApi";
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-3">
              ...
              <span
                className={`text-sm font-bold ${isUp ? "text-[#10B981]" : "text-[#EF4444]"}`}
              >
                {change}
              </span>
...
export function DashboardPage({
  ...
  const tenantId = useSelector(
    (state: RootState) => state?.auth?.user?.tenant_id as string,
  );
  const { data, isLoading } = useGetListAirTicketQuery(tenantId);
  const [createAirTicket, { isLoading: isCreating }] =
    useCreateAirTicketMutation();

  const handleCreate = async () => {
    try {
      await createAirTicket({ name: "New Ticket", tenantId }).unwrap();
    } catch (err) {
      console.error(err);
    }
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-base text-muted-foreground mt-1">
            Biomedical equipment overview and KPIs
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Badge
            variant="outline"
            className="text-sm py-1.5 px-4 border-[#10B981]/30 text-[#10B981] bg-[#10B981]/5 font-semibold"
          >
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader className="pb-3 px-7 pt-6 flex flex-row items-center justify-between">
          ...
          <Button
            variant="ghost"
            size="sm"
            className="text-[#00BCD4] hover:text-[#00838F] text-sm font-semibold"
            onClick={() => onNavigate(ROUTES.WORK_ORDERS)}
          >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-border">
                  <th className="text-left py-4 px-4 font-bold text-foreground text-sm">
                    WO #
                  </th>
                  <th className="text-left py-4 px-4 font-bold text-foreground text-sm">
                    Equipment
              ...
              <tbody>
                {recentWorkOrders.map((wo) => (
                  ...
                  >
                    <td className="py-4 px-4 font-mono font-bold text-[#00BCD4] text-sm">
                      {wo.id}
                    </td>
                    <td className="py-4 px-4 text-foreground text-sm font-medium max-w-[240px] truncate">
                      {wo.equipment}
                    </td>
                    <td className="py-4 px-4">
                      <span
                        className={`inline-flex px-3 py-1 rounded-full text-sm font-semibold border ${priorityColors[wo.priority]}`}
                      >
                        {wo.priority}
                      </span>
                    </td>
                    <td className="py-4 px-4">
                      <span
                        className={`inline-flex px-3 py-1 rounded-full text-sm font-semibold ${statusColors[wo.status]}`}
                      >
                        {wo.status}
                      </span>
                    </td>
