...
import { inventoryItems } from "@/src/lib/mock-data";
        ...
        <div className="flex gap-3">
          {lowStockCount > 0 && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#FEF3C7] text-[#D97706] text-sm font-bold">
              <AlertTriangle className="w-4 h-4" /> {lowStockCount} items need
              attention
            </div>
          )}
          <Button
              ...
              <tbody>
                {inventoryItems.map((item) => (
                  ...
                  >
                    ...
                    <td
                      className={`py-4 px-5 font-bold text-sm ${item.qty <= item.minQty ? "text-[#DC2626]" : "text-foreground"}`}
                    >
                      {item.qty}
                    </td>
                    <td className="py-4 px-5 text-foreground text-sm">
                    ...
                    <td className="py-4 px-5 text-foreground font-semibold text-sm">
                      ${item.unitCost.toLocaleString()}
                    </td>
                    <td className="py-4 px-5">
                      <span
                        className={`inline-flex px-3 py-1 rounded-full text-sm font-semibold ${statusColors[item.status]}`}
                      >
                        {item.status}
                      </span>
                    </td>
